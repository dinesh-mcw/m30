...
use_old_metadata_format = False
fov_num_to_use = 0
file_name_base = 'wall_ci20_angular02_0_01_'
input_data_name, input_data_shape, perform_tap_add =\
...
for frame_num in range(len(data_gen_1p0.data[input_data_name])):
    ...
    # output_range_array_name = device_0p2(input_data=input_data, roi_start_vector=start_vector_0p2, configs=configs)
    # range_map_0p2 = np.copy(device_0p2.dsp.data[output_range_array_name].flt)
    # range_map_0p2_active = np.copy(range_map_0p2)
    # RANGE_0p2.append(np.copy(device_0p2.dsp.data[output_range_array_name].flt))
    #
    # saved = np.copy(range_map_0p2_active[active_region[0]:active_region[1], active_region[2]:active_region[3]])
    # range_map_0p2_active[:, :] = np.nan
    # range_map_0p2_active[active_region[0]:active_region[1], active_region[2]:active_region[3]] = saved
    #
    # unwrap_errors_mask = np.abs(range_map_0p2_active[~np.isnan(range_map_0p2_active)] - 6.617894288788537) > phase_unwrapping_error_range_bracket
    # phase_unwrapping_error = 100*np.count_nonzero(unwrap_errors_mask) / max(len(range_map_0p2_active[~np.isnan(range_map_0p2_active)]), 1)
    # phase_unwrapping_errors_0p2.append(phase_unwrapping_error)
    # precision_range_array = range_map_0p2_active[np.invert(SNR_thresholding_mask)].flatten()
    # precision = 100 * np.std(precision_range_array) / 6.617894288788537
    # precision_0p2.append(precision)
    #
    # snr_map_0p2 = np.copy(np.mean(device_0p2.dsp.data["SNR_frame"].flt, axis=0))
    # SNR_0p2.append(snr_map_0p2)
    # SNR_0p2_smoothed.append(np.copy(np.mean(device_0p2.dsp.data["SNR_frame_smoothed"].flt, axis=0)))
...
# SNR_0p2_smoothed = np.mean(np.asarray(SNR_0p2_smoothed), axis=0) # take the mean across all frames
